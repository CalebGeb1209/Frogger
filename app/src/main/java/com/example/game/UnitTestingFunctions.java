...
public class UnitTestingFunctions {
    ...
    public static int DecreaseLives(int currentLives, int playerXCoordinate, int playerYCoordinate) {
    ...
    public static int changeScore(int currentScore, int playerXCoordinate, int playerYCoordinate) {
        ...
        * This function resets the score back to zero if we go into a water tile if we have 3 or 2 lives and keeps current score if we have 1 life (game over)
         */
        if (DecreaseLives(3, playerXCoordinate, playerYCoordinate) == 2 || DecreaseLives(2, playerXCoordinate, playerYCoordinate) == 1) {
            currentScore = 0;
        } else if (DecreaseLives(1, playerXCoordinate, playerYCoordinate) == 0) {
            return currentScore;
        }
        return currentScore;
    }
